...
class ReleasesTable extends Component {
  state = {
    ...
    page: 0,
    rowsPerPage: 10,
    currentlyDisplayed: this.props.state.auctions
  }

  componentWillMount() {
    const fullList = this.props.state.auctions.auctions;
  ...
  handleClick = (event, row) => {
    const selectedAuction = this.props.state.auctions.find(a => (a.saleNumber === row.saleNumber));

    this.props.actions.selectAuction(selectedAuction);
    this.props.actions.loadSelectedAuctionRequested();
    this.props.actions.lotsGetSubmit();
    this.props.actions.highlightsGetSubmit();
    this.props.actions.getLastWinnigBidEmailStatusSubmit();
    this.props.actions.getAMSaleSubmit();
  }
  ...
  filterOnInputChange = (event) => {
    let newlyDisplayed = filter(
      this.props.state.auctions,
      auction => (
        auction.auctionTitle.toLowerCase().includes(event.target.value)
        || auction.saleNumber.toLowerCase().includes(event.target.value)
        || auction.auctionTitle.toUpperCase().includes(event.target.value)
        || auction.saleNumber.toUpperCase().includes(event.target.value)
      )
    );
          ...
          <Table className={classes.table}>
            ...
            <TableBody>
              {stableSort(auctionsWithId, getSorting(order, orderBy))
                ...
                .map(row => {
                  const isSelected = this.props.state.selectedAuction.auctionPublicID === row.auctionPublicID;
                  return (
                    <TableRow
                      hover
                      key={row.id}
...
ReleasesTable.propTypes = {
  ...
  actions: PropTypes.shape({
    ...
    loadSelectedAuctionRequested: PropTypes.func.isRequired,
    getLastWinnigBidEmailStatusSubmit: PropTypes.func.isRequired,
    highlightsGetSubmit: PropTypes.func.isRequired,
    lotsGetSubmit: PropTypes.func.isRequired,
    selectAuction: PropTypes.func.isRequired,
    getAMSaleSubmit: PropTypes.func.isRequired
  }).isRequired
}
