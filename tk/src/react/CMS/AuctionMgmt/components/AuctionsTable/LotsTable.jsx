...
class LotsTable extends Component {
  state = {
    ...
    page: 0,
    rowsPerPage: 10,
    currentlyDisplayed: this.props.state.lots.lots
  }
  ...
  handleClick = (event, row) => {
    const selectedLot = this.props.state.lots.lots.find(a => (a.lotNumber === row.lotNumber))
  ...
  filterOnInputChange = (event) => {
    let newlyDisplayed = filter(
      ...
      lot => (
        lot.makerName.toLowerCase().includes(event.target.value)
        || lot.makerName.toUpperCase().includes(event.target.value)
        || lot.description.toLowerCase().includes(event.target.value)
        || lot.description.toUpperCase().includes(event.target.value)
      )
    );
          ...
          <Table className={classes.table}>
            ...
            <TableBody>
              {stableSort(lotsWithId, getSorting(order, orderBy))
                ...
                .map(row => {
                  const isSelected = this.props.state.selectedLot.auctionLotPublicId === row.auctionLotPublicId;
                  return (
                    <TableRow
                      hover
                      key={row.id}
