...
const getDetails = props => validators => Object.keys(validators)
...
const OriginalContent = props => (
  <li className="lot-page__details__list__item lot-page__details__list__item--lot-details" key="lot-details" name={props.name}>
    <ul className="lot-page__details__list lot-page__details__list--sticky">
      {props.children}
    </ul>
  </li>
);
const StickyDetails = (props) => {
  ...
  if (props.additionalContent.length > 0) {
    props.additionalContent.forEach((content) => {
      children.push(<AdditionalContent {...content} key={`${content.objectNumber} - ${content.componentContainerId}`} />);
  ...
  return (
    <StickyContainer className="lot-page__details sticky-container sticky-container--details-container">
      <div ref={stickyNavRef}>
        <Sticky
          topOffset={-clearanceHeight}
          bottomOffset={clearanceHeight}
        >
          {(stickyProps) => {
            const styles = stickyProps.isSticky
              ? { ...stickyProps.style, top: stickyProps.style.top + clearanceHeight, borderBottom: '1px solid #ccc', borderTop: 'none' }
              : stickyProps.style;
            return (
              <ul
              ...
              >
                {navLinks.map((title) => {
                  ...
                  return (
                    <li
                      className="sticky-container__sticky-wrapper__sticky-wrapper-item"
                      key={title}
                    >
                      <Link
      ...
      <ul className="lot-page__details__list lot-page__details__list--has-nav">
        {children}
      </ul>
    </StickyContainer>
  );
