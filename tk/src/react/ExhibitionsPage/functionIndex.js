...
export const generateFilterPayload = (selected = false, currentFilterValues = [], filterQuery = {}, parameter = '', value = '') => {
  ...
  if (selected) {
    ...
    // console.log('SElECTED REMOVE', filterPayload);
  } else {
    ...
    // console.log('NOT SELECTED ADD', filterPayload);
  }
...
export const auctionDetailSanitizer = (detail) => {
  ...
  const badCharsArray = ['<', '&', '>', ';', '/'];
  const replaceArray = ['<br />', '&nbsp;', '</p>', '<p>', '&amp', ';', '&ndash', '&#8211', '<br />'];
  if (detail && contains(detail, badCharsArray)) {
...
export const filterItems = (data, filterparam) => {
  if (!filterparam.filter && (filterparam.sort === 'oldest')) {
    return data.sort((a, b) => new Date(a.eventDate) - new Date(b.eventDate));
  } if (filterparam.filter && filterparam.sort === 'oldest') {
    const parsedExhibitions = parseFilterQuery(filterparam.filter);
    return createFilteredItems(data, parsedExhibitions).sort((a, b) => new Date(a.eventDate) - new Date(b.eventDate));
  } if (filterparam.filter && filterparam.sort === '') {
    const parsedExhibitions = parseFilterQuery(filterparam.filter);
    return createFilteredItems(data, parsedExhibitions).sort((a, b) => new Date(b.eventDate) - new Date(a.eventDate));
  } if (filterparam.filter && filterparam.sort === 'newest') {
    const parsedExhibitions = parseFilterQuery(filterparam.filter);
    return createFilteredItems(data, parsedExhibitions).sort((a, b) => new Date(b.eventDate) - new Date(a.eventDate));
  }
  return data.sort((a, b) => new Date(b.eventDate) - new Date(a.eventDate));
};
